...
from fusion_imm import run_imm_fusion  # IMM Modülü
...
def step1_generate_data():
    """Ground truth verisinden idealize ve gerçekçi radar simülasyon verileri üretir."""
...
def step2_run_scenarios():
    """Tüm füzyon senaryolarını çalıştırır."""
    print("\n[AŞAMA 2] Füzyon senaryoları çalıştırılıyor...")

    # İdealize Senaryolar (Referans / Baseline)
...
def step3_compute_metrics():
    ...
    # Tüm senaryoların çıktılarını sözlüğe ekle
    dfs = {
        "İdealize + Temel (CV)": _safe_read_csv(RES_IDEAL_BASIC),
        "İdealize + Gelişmiş (CA+CI)": _safe_read_csv(RES_IDEAL_ADV),
...
def step4_compute_target_metrics():
    ...
    for target in targets:
        print("=" * 105)
        print(f"HEDEF: {target}".center(105))
        print("=" * 105)
